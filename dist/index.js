...
node_cron_1.default.schedule('0,15 * * * *', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const event of events_json_1.default) {
        ...
        if (timeLag >= -60000 && timeLag <= 600000) {
            const mentionRole = (_b = (_a = client.guilds.cache
                .get('755774191613247568')) === null || _a === void 0 ? void 0 : _a.roles.cache.filter((role) => role.name.includes(event.role)).first()) === null || _b === void 0 ? void 0 : _b.id;
            const notifyChannel = client.channels.cache.get('805732155606171658');
            notifyChannel === null || notifyChannel === void 0 ? void 0 : notifyChannel.send(`<@&${mentionRole}> ${event.name}`);
        }
    }
...
setInterval(() => {
    Bot_1.default.db.query('SELECT * FROM `threadCloseQueue`', (e, rows) => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
        if (!rows || !rows[0])
            return;
        for (const row of rows) {
            if (row.date <= Date.now()) {
                ...
                yield channel.setParent(config_json_1.default.thread.closedCategory);
                channel.send({
                    embeds: [
                        new discord_js_1.MessageEmbed()
                ...
                // スレッド一覧の埋め込み色を赤色にする
                (_a = client.channels.cache.get(config_json_1.default.thread.createChannel)) === null || _a === void 0 ? void 0 : _a.messages.fetch(rows[0].listMessageId).then((msg) => {
                    msg.edit({
                ...
                // チャンネルを空きチャンネルとしてマーク
                Bot_1.default.db.query('UPDATE `threadChannels` SET `inUse` = ? WHERE `channelId` = ?', [false, row.channelId]);
                // スレッドをClose済としてマーク
                Bot_1.default.db.query('UPDATE `threads` SET `closed` = ? WHERE `channelId` = ? AND `closed` = ?', [true, row.channelId, false]);
            }
        }
    }));
}, 600000);
