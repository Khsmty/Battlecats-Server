    ...
    if (interaction.isCommand()) {
      ...
      switch (interaction.customId) {
        case 'chrole': {
          ...
          const userRoles = interaction.member!.roles.cache.map((role) => role.id).filter((f) => chRoles.includes(f));
          ...
          else interaction.member!.roles.remove(userRoles.filter((i) => interaction.values.indexOf(i) === -1));
        ...
        case 'pgrole': {
          ...
          await interaction.member!.roles.remove(pgRoles.filter((i) => interaction.values.indexOf(i) === -1));
        ...
        case 'gmrole': {
          ...
          const userRoles = interaction.member!.roles.cache.map((role) => role.id).filter((f) => gmRoles.includes(f));
