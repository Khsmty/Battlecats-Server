...
module.exports = {
  ...
  async execute(message: Message, args: any[]) {
    ...
    if (args[0] === 'add') {
      ...
      const sendImage = await message.client.channels.cache.get('893791664278220840').send({
        files: [
          {
      ...
      message.client.db.query(
        ...
        async (e, rows) => {
          if (!rows[0]) {
            message.client.db.query('INSERT INTO `dojo` (`userId`, `type`, `imageUrl`, `score`) VALUES (?, ?, ?, ?)', [
              args[2],
              args[1],
