...
module.exports = {
  ...
  async execute(message: Message, args: any[]) {
    if (!message.member?.roles.cache.has('868797324699242507') && !message.member?.permissions.has('ADMINISTRATOR'))
      return;
    ...
    if (args[0] === 'add') {
      ...
      Bot.db.query('SELECT * FROM `dojo` WHERE `userId` = ? AND `type` = ?', [args[2], args[1]], async (e, rows) => {
        if (!rows[0]) {
          Bot.db.query('INSERT INTO `dojo` (`userId`, `type`, `imageUrl`, `score`) VALUES (?, ?, ?, ?)', [
            args[2],
            args[1],
            sendImage.attachments.first().url,
            args[3],
          ]);
          message.reply('スコアを新規登録しました。');
        } else {
          Bot.db.query('UPDATE `dojo` SET `imageUrl` = ?, `score` = ? WHERE `userId` = ? AND `type` = ?', [
            sendImage.attachments.first().url,
            args[3],
            args[2],
            args[1],
      ...
      Bot.db.query('SELECT * FROM `dojo` WHERE `type` = ? ORDER BY `score` DESC', [args[1]], async (e, rows) => {
